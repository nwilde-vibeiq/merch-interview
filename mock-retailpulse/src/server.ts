...
app.post("/__admin/config", (req, res) => {
  ...
  if (requested !== 1 && requested !== 2) {
    return res
      .status(400)
      .json({ error: 'Body must be { "apiVersion": 1 | 2 }' });
  }
...
app.get("/v1/sales", (req, res) => {
  ...
  if (typeof sku !== "string" || sku.length === 0) {
    return res
      .status(400)
      .json({ error: "Missing required query parameter 'sku'" });
  }
  ...
  if (!records) {
    return res
      .status(404)
      .json({ error: `No sales feed configured for sku '${sku}'` });
  }
  ...
  res.json({
    ...
    records: page,
    next_page_token:
      nextOffset < records.length ? encodeToken(nextOffset) : null,
  });
});

app.listen(PORT, () => {
  console.log(
    `RetailPulse mock listening on http://localhost:${PORT} (apiVersion ${apiVersion})`
  );
});
